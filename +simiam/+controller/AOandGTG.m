...
%             obj.p = simiam.util.Plotter();
        ...
        function outputs = execute(obj, robot, state_estimate, inputs, dt)
            ...
            s=sqrt(u_ao(1)^2+u_ao(2)^2);
            u_ao_n=[u_ao(1)/s;u_ao(2)/s];
            
            x=sqrt(u_gtg(1)^2+u_gtg(2)^2);
            u_gtg_n=[u_gtg(1)/x;u_gtg(2)/x];
            a=0.75;
            u_ao_gtg = a*u_ao_n+(1-a)*u_gtg_n;
